...
export function EmployeeModal({ employee, onSave, onClose }) {
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <label>Proficiency:
        <select id="emp-proficiency">
          <option value="Beginner" ${employee?.proficiency === "Beginner" ? "selected" : ""}>Beginner</option>
          <option value="Intermediate" ${employee?.proficiency === "Intermediate" ? "selected" : ""}>Intermediate</option>
          <option value="Advanced" ${employee?.proficiency === "Advanced" ? "selected" : ""}>Advanced</option>
        </select>
      </label><br>
  ...
  modal.querySelector("#save-btn").onclick = () => {
    ...
    const proficiency = modal.querySelector("#emp-proficiency").value;
    const last_updated = modal.querySelector("#emp-last-updated").value;
    onSave({ ...employee, name, role, proficiency, last_updated });
  };
